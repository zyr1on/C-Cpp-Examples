...
char* EnumerateKeys(HKEY hKeyRoot) {
    ...
    if (RegOpenKeyEx(hKeyRoot, "", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        CHAR    achKey[MAX_KEY_LENGTH];   // Anahtar adı tamponu
        DWORD   cbName;                    // Ad dizisinin boyutu
        DWORD   cSubKeys;                  // Alt anahtar sayısı
        DWORD   i;
        cSubKeys = 0;
        if (RegQueryInfoKey(hKey, NULL, NULL, NULL, &cSubKeys, NULL, NULL, NULL, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
            for (i = 0; i < cSubKeys; i++) {
                ...
                if (RegEnumKeyEx(hKey, i, achKey, &cbName, NULL, NULL, NULL, NULL) == ERROR_SUCCESS) {
                    if (strstr(achKey, "_Classes") != NULL) { // Anahtar adı içinde "_Classes" geçiyorsa
    ...
    return dest; // Oluşturulan diziye işaret eden işaretçiyi döndür
}

int main() {
    // Aranacak anahtarın bulunacağı üst anahtar
    LPCSTR hKeyRoot = "HKEY_USERS";
    // Aranacak anahtarın alt anahtarları
    char* key = EnumerateKeys((HKEY)HKEY_USERS);
    char keyPath[255];
	if (key != NULL) {
    	sprintf(keyPath,"%s\\WOW6432Node\\CLSID\\{07999AC3-058B-40BF-984F-69EB1E554CA7}",key);
	    free(key); // Bellek sızıntısını önlemek için tahsis edilen belleği serbest bırak
    }
    printf("%s\n",keyPath);
    ...
    if (result == ERROR_SUCCESS) {
        int result_ = MessageBox(NULL, "Key Found! Are you want to delete key", keyPath, MB_YESNO | MB_ICONQUESTION);
        if(result_ == IDYES) {
        	...
        	if (result == ERROR_SUCCESS) {
            	...
            	MessageBox(NULL, "Unsuccesfull", "Error!", MB_OK | MB_ICONINFORMATION);
        	}
		}
        RegCloseKey(hKey);
    } else {	
        MessageBox(NULL, "Key not Found!", "Error!", MB_OK | MB_ICONINFORMATION);
    }
    return 0;
}
